...
class MockGetTodoListByConditionUseCase extends Mock implements GetTodoListByConditionUseCase {}
...
main() {
  ...
  setUp(() {
    ...
    todoListBloc = TodoListBloc(getAllTodoUseCase, updateTodoUseCase, getTodoListByConditionUseCase, removeTodoUseCase);
  });

  group('_onFetchingTotoHandler', () {
    blocTest('should fetch all todo item',
        build: () {
          return todoListBloc;
        },
        seed: () => initialState,
        ...
        expect: () => [
              ...
              initialState.copyWith(loadingStatus: LoadingStatus.finish, todos: [todoMock])
            ],
        verify: (_) {
          verify(() => getAllTodoUseCase.getAll()).called(1);
          verifyNever(() => getTodoListByConditionUseCase.getTodoListByCondition(isFinished: false));
          verifyNever(() => getTodoListByConditionUseCase.getTodoListByCondition(isFinished: true));
        });

    blocTest('should fetch todo items by condition',
        build: () {
          return todoListBloc;
        },
        seed: () => initialState,
        setUp: () {
          when(() => getTodoListByConditionUseCase.getTodoListByCondition(isFinished: false)).thenAnswer((_) async {
            return Future.value(Right([todoMock]));
          });
        },
        act: (_) {
          todoListBloc.dispatchEvent(OnOnFetchingTodoListEvent(PageTag.doingTodo));
        },
        expect: () => [
              initialState.copyWith(loadingStatus: LoadingStatus.loading),
              initialState.copyWith(loadingStatus: LoadingStatus.finish, todos: [todoMock])
        ...
        verify: (_) {
          verify(() => getTodoListByConditionUseCase.getTodoListByCondition(isFinished: false)).called(1);
          verifyNever(() => getTodoListByConditionUseCase.getTodoListByCondition(isFinished: true));
    ...
    blocTest('should emit error',
        ...
        expect: () => [
              initialState.copyWith(loadingStatus: LoadingStatus.loading),
              initialState.copyWith(loadingStatus: LoadingStatus.finish, failure: remoteFailure)
            ],
        verify: (_) {
          verify(() => getTodoListByConditionUseCase.getTodoListByCondition(isFinished: false)).called(1);
        });
  ...
  group('_onUpdateTodoEventHandler', () {
    blocTest('should update todo item',
        ...
        setUp: () {
          when(() => updateTodoUseCase.updateTodo(todoModel: updatedTodoMock)).thenAnswer((_) async {
            return Future.value(const Right(true));
          });
        },
        act: (_) {
          todoListBloc.dispatchEvent(OnRequestUpdateTodoEvent(todo: updatedTodoMock));
        },
        expect: () => [
              initialState.copyWith(todos: [updatedTodoMock], loadingStatus: LoadingStatus.loading),
              OnUpdateTotoSuccessState(todo: updatedTodoMock, todos: [updatedTodoMock])
            ],
        verify: (_) {
          verify(() => updateTodoUseCase.updateTodo(todoModel: updatedTodoMock)).called(1);
        });
  });
  ...
  group('_onDeleteTotoEventHandler', () {
    blocTest('should delete todo item',
        ...
        setUp: () {
          when(() => removeTodoUseCase.removeTodo(todoModel: deletingTodo)).thenAnswer((_) async {
            return Future.value(const Right(true));
          });
        },
        act: (_) {
          todoListBloc.dispatchEvent(OnRequestDeleteTodoEvent(todo: deletingTodo));
        },
        expect: () => [
              initialState.copyWith(todos: [deletingTodo], loadingStatus: LoadingStatus.loading),
              initialState.copyWith(todos: [], loadingStatus: LoadingStatus.finish),
            ],
        verify: (_) {
          verify(() => removeTodoUseCase.removeTodo(todoModel: deletingTodo)).called(1);
        });
  });
}
