...
main() {
  ...
  final initialState = MainState(loadingStatus: LoadingStatus.none, failure: null);
  final addNewTodo = TodoModel(id: 0, title: 'title', description: 'description', createdDate: DateTime.now(), isFinished: true);
  ...
  blocTest('should emit AddNewTodoSuccessState when new item was added successfully',
      seed: () => initialState,
      setUp: () {
        when(() => usecase.addNewTodo(todoModel: addNewTodo)).thenAnswer((_) async {
  ...
  blocTest('should emit failure error',
      seed: () => initialState,
      setUp: () {
        when(() => usecase.addNewTodo(todoModel: addNewTodo)).thenAnswer((_) async {
